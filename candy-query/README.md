...
| `Sampler` | Rate computer service: takes two status snapshots and elapsed time, computes per-second deltas (Bytes_received, Bytes_sent, etc.) with negative-delta clamping for restart detection. Advance via `poll()` on each `withRefresh()` call. |
