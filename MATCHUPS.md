...
| [charmbracelet/x/xpty](https://github.com/charmbracelet/x/tree/main/xpty) | **CandyPty** | `candy-pty/` | `sugarcraft/candy-pty` | `SugarCraft\Pty` | 🟢 | PTY primitive (Linux + macOS) — `Pty::open / spawn / read / write / resize / size / setBlocking`, `Child` lifecycle, `SignalForwarder` (SIGWINCH → resize), opt-in TIOCSCTTY via `controllingTerminal: true` (Ctrl+C → SIGINT delivery via `bin/pty-shim.php`); all via FFI to libc |
