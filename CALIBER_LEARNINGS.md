...
- **[pattern:rebuilddecoderat-single-helper]** `Player::rebuildDecoderAt(int $cellsW, int $cellsH, Mode $mode, int $frameIndex)` is the single helper for ALL decoder recreation: backward-seek, mode-switch, and WindowSizeMsg resize. Real path: `$this->decoder->close()` then `DecoderFactory::create()` (prevents F21 — backward-seek process leak). Test path (`/fake`): re-opens the injected decoder. Every code path that creates a new decoder must use this helper.
