...
| 1 | CandyGloss | 🟡 | 50% | `Style` (attrs, fg/bg, padding, margin, width/height, horizontal align, profile-aware downsampling) + `Border` (normal/rounded/thick/double/block/ascii/hidden + custom). Inherit, vertical align, table/list/tree TBD. |
| 2 | HoneyBounce | 🟢 | 100% | `Spring` (under-/critically-/over-damped) + `Spring::fps()`. Pure math, ready for downstream use. |
| 3 | CandyCore (runtime) | 🟡 | 40% | **ReactPHP/event-loop chosen.** `Model`, `Msg`, `Cmd` (helpers: quit/batch/send), `KeyType`, `Program`, `ProgramOptions`, `Renderer`, `InputReader`. Core messages: `KeyMsg`, `QuitMsg`, `WindowSizeMsg`. Input parsing covers printable ASCII, ctrl, alt-prefix, arrows, Home/End/Delete/PgUp/PgDn. TBD: mouse, focus/blur, bracketed paste, OSC, function keys, diff renderer. |
| 4 | CandyZone | 🔴 | 0% | |
