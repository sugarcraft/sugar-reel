...
| [`x/windows`](https://github.com/charmbracelet/x/tree/main/windows) | Windows TTY backend (raw mode, resize, Ctrl+C handler) | 🟢 | Ported to `candy-core/src/Util/Tty/WindowsBackend.php` — FFI scaffold to kernel32.dll, enableRawMode/restore with ENABLE_VIRTUAL_TERMINAL_PROCESSING, resize via poll loop, Ctrl+C/Break handler via SetConsoleCtrlHandler, mintty/WSL/Cygwin detection, CONIN$/CONOUT$ direct handle opening, ReadConsoleInputW for interrupt polling. Requires Windows 10 1809+. All PRs merged (PR1-PR7). |
| [`x/vt`](https://github.com/charmbracelet/x/tree/main/vt) | In-memory virtual terminal emulator (ANSI byte stream → cell grid + cursor + mode state) | 🟢 | Ported to `candy-vt/` — VT500 Paul-Williams parser (state machine + transition table generated to match charmbracelet/x/ansi/parser); ScreenHandler dispatches CSI/ESC/OSC; full SGR (16/256/truecolor + attrs); cursor moves with DECSC/DECRC; erase + scroll; DEC modes 25/1000/1002/1003/1006/1049/2004/2026; OSC 0/1/2 title, 4 palette, 8 hyperlinks, 52 clipboard; tab stops with HTS/TBC/CHT/CBT + `Terminal::withTabStops()`; Width-aware printChar (CJK continuation cells); fixtures + fuzzer. All PRs merged (PR1-PR9). |
| [`x/vcr`](https://github.com/charmbracelet/x/tree/main/vcr) | Record + replay candy-core Program sessions (bug repro / regression / fuzzing) | 🟢 | Ported to `candy-vcr/` — Cassette + JSONL/YAML formats; `Recorder` interface in candy-core + impl in candy-vcr; `Program::withRecorder()` tees input bytes / output bytes / resize / quit; 14 candy-core Msg serializers + JsonSerializable catch-all + Registry; `Player` chained-dispatch through cassette events with INSTANT/REALTIME modes; `ByteAssertion` + `ScreenAssertion` (cell-grid via candy-vt); `bin/candy-vcr` CLI with `inspect / replay / diff` subcommands; examples + counter cassette fixture. All PRs merged (PR1-PR7). |
